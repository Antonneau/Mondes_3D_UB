...
{
    // E1 = A - C
    Vector3f E1 = vertexOfFace(faceId, 0).position - vertexOfFace(faceId, 2).position;
    // E2 = B - C
    Vector3f E2 = vertexOfFace(faceId, 1).position - vertexOfFace(faceId, 2).position;
    // T = O - C
    Vector3f T = ray.origin - vertexOfFace(faceId, 2).position;
    // P = D x E2
    Vector3f P = ray.direction.cross(E2);
    // Q = T x E1
    Vector3f Q = T.cross(E1);

    float PE1 = 1 / (P.dot(E1));
    float u = PE1 * (P.dot(T));
    float v = PE1 * (Q.dot(ray.direction));
    float t = PE1 * (Q.dot(E2));

    // If the intersection is not in the mesh (u, v not in [0, 1])
